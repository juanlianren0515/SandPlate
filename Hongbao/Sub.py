...
from Text.Material import add_qun_note, note_to_individual
from Room.Sub import get_all_win_by_name
from SendMsgByQQ.QQGUI import send_qq, send_qq_pic, send_qq_hwnd_click_string, send_qq_pic_hwnd_click_string
...
def send_single_product_to_single_qun(product_dict, win_hwnd, pic_st, text_st):
	...
	if len(product_dict['product_image_list']) > 0:
		for image in product_dict['product_image_list']:
			send_qq_pic_hwnd_click_string(win_hwnd, image)
			time.sleep(pic_st)

	# 发送buy key
	send_qq(win_hwnd, product_dict['product_buy_key'])
	time.sleep(text_st)
...
def send_all_product_to_all_qun(qun_list_, product_list, text_st, pic_st, product_st):
	...
	for qun in qun_list_:
		...
		# 附带加qun推广
		send_qq_hwnd_click_string(qun, note_to_individual)
